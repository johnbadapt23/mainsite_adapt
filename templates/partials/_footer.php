...
<?php } else { ?> 
	<footer>
		<div class="container">
			<div class="footer-top">
				<div class="footer-column-container group-column">
					<?php if ( have_rows( 'footer_column_groups', 'options'  ) ) : ?>
						<?php while ( have_rows( 'footer_column_groups', 'options'  ) ) : the_row(); ?>
							<div class="footer-column column one-third">
								<?php if ( have_rows( 'footer_columns'  ) ) : ?>
									<?php $counter=1; ?>
										<?php while ( have_rows( 'footer_columns'  ) ) : the_row(); ?>
											<?php $portal_link = 'no'; ?>
											<?php if ( get_sub_field( 'portal_link' ) == 1 ) {  ?>
												<?php $portal_link = 'yes'; ?>
											<?php } ?>
											<div class="footer-column-group">
												<span class="footer-column-title-wrapper"><span class="footer-column-title"><?php echo get_sub_field( 'column_title' ); ?></span></span>
												<?php if ( have_rows( 'link' ) ) : ?>
													<span class="footer-link-container-wrapper">
														...
														<?php endwhile; ?>
														<?php if ($portal_link == 'yes'){ ?>
															<a class="footer-link portal text-red external-link" href="<?php echo $accessLink; ?>" target="_blank" rel="noopener noreferrer"><?php echo $accessLinkText; ?></a>
														<?php } ?>
														</span>
												<?php else : ?>
													<?php // no rows found ?>
												<?php endif; ?>
												<?php if ( get_sub_field( 'socials' ) == 1 ) {  ?>
													<span class="socials-container">
														<span class="footer-column-title follow-us-title">Follow Us</span>
														<span class="social-links">
															<?php if ($linkedInLink) {?>
																<a class="social-link linkedin" href="<?php echo $linkedInLink;?>" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
															...
															<?php } ?>
															<?php if ($youtubeLink) {?>
															<a class="social-link youtube" href="<?php echo $youtubeLink;?>" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
																<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					...
					<div class="subscribe-sidebar-form background-tertiary-black new-layout">
						...
						<?php if ( have_rows( 'subscribe_button', 'options') ) : ?>
							<?php while ( have_rows( 'subscribe_button', 'options' ) ) : the_row(); ?>
								<a class="std-button red-outline-button button-with-arrow red-arrow-button" href="<?php echo get_sub_field( 'button_link' ); ?>"><?php echo get_sub_field( 'button_text' ); ?></a>							
							<?php endwhile; ?>
						<?php else : ?>
							<?php // no rows found ?>
						<?php endif; ?>
				...
				<div class="logo-container">
					...
					<?php if ( $footer_icon ) { ?>
						<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
							<?php echo wp_get_attachment_image( $footer_icon['ID'], 'adapt-optimized', false, array(
								'class'   => 'logo',
								'alt'     => $footer_icon['alt'],
								'loading' => 'lazy',
							) ); ?>
					...
					<?php while ( have_rows( 'footer_bottom', 'options' ) ) : the_row(); ?>
						<div class="footer-bottom-links-container">
							<div class="footer-bottom-left">
								<span class="footer-bottom-text text-medium-grey"><?php echo get_sub_field( 'footer_bottom_text' ); ?></span>
							</div>
							<div class="footer-bottom-right">								
								<?php if ( have_rows( 'footer_bottom_links' ) ) : ?>
									<?php while ( have_rows( 'footer_bottom_links' ) ) : the_row(); ?>
										<a class="footer-bottom-link text-medium-grey" href="<?php echo get_sub_field( 'link' ); ?>" target="<?php echo get_sub_field( 'link_target' ); ?>"><?php echo get_sub_field( 'link_text' ); ?></a>
									<?php endwhile; ?>
								<?php else : ?>
									<?php // no rows found ?>
								<?php endif; ?>
	...
	<span class="subscribe-form-container" style="display:none;"><?php echo get_field( 'form_embed', 'options' ); ?></span>
